...
const transformArticle = (item, type = 'news') => {
  if (type === 'heading') {
    return {
      ...
      author: { name: item.author || item.blog_by || 'समाचार टोली', avatar: null },
      views: item.views || 0,
      content: item.content || item.description || '<p>सामग्री उपलब्ध छैन</p>',
      category: item.category
        ? { name: item.category, slug: item.category.toLowerCase().replace(/[^\w]/g, '-'), color: getCategoryColor(item.category) }
        : { name: 'सामान्य', slug: 'general', color: '#8B0000' },
      tags: item.tags || [],
    };
  }
  ...
  return {
    ...
    category: { name: categoryName, slug: categorySlug, color: getCategoryColor(categoryName) },
    tags: item.tags || [],
...
const transformRelated = (item) => {
  ...
  return {
    ...
    category: { name: categoryName, color: getCategoryColor(categoryName) },
  };
};
...
// ─── NEWS DETAIL PAGE ─────────────────────────────────────────────────────────
// SEO STRATEGY: title, description, canonical are set server-side in the
// Laravel controller via the 'seo' prop → app.blade.php CASE 1 prints them.
// This <Head> block only handles the 404 noindex case — no duplicate tags.
const NewsDetailPage = ({ article, related = [], slug, ads = {}, seo = {} }) => {
  ...
  // ── 404 / empty state ─────────────────────────────────────────────────────
  // Only case where JSX <Head> sets SEO — no seo prop from controller here
  if (!transformedArticle) {
    return (
      <>
        <Head>
          <title>समाचार फेला परेन</title>
  ...
  // ── No <Head> here — Blade already printed title/desc/canonical server-side
  // via the 'seo' prop from the controller (app.blade.php CASE 1)
  return (
    <>
      {/* ── PAGE CONTENT ───────────────────────────────────────────────────── */}
      <Navbar />
